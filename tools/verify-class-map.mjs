...
for(const removed of ['UPandoraInstance','UAbilityAttributeManager','UProjectileAbility'])assert(!nodes.has(removed),`Deleted class still selected: ${removed}`);
assert(nodes.get('UPandoraTreeComponent').fields.find(f=>f.key==='GrantedPandoras').schemas[0].fields.some(f=>f.key==='Level'));
assert(nodes.get('UPandoraDefinition').fields.find(f=>f.key==='Skills').references.includes('USkillDefinition'));
assert(!nodes.get('UPandoraDefinition').fields.some(f=>f.key==='Skill'));
assert.deepEqual(nodes.get('UPandoraSkillSource').fields.map(f=>f.key),['PandoraDefinition','SkillIndex','PandoraLevel','LoadoutDirection']);
assert(nodes.get('USkillDefinition').fields.find(f=>f.key==='Action').references.includes('USkillAction'));
assert(nodes.get('USkillAbility').fields.find(f=>f.key==='ActiveAction').references.includes('USkillAction'));
assert(!nodes.get('USkillDefinition').fields.some(f=>f.key==='AbilitiesToGrant'||f.key==='ProjectileSettings'));
assert(!nodes.get('UPdAbilitySystemComponent').fields.some(f=>/AttributeManager|Cooldown/i.test(f.key)));
assert.equal(nodes.get('UAbilityCostAndCooldownManager').fields.length,0);
assert(nodes.get('UStatusEffectDefinition').fields.some(f=>f.key==='StackGameplayEffectClass'));
...
for(const id of ['FPandoraSkillBinder>USkillAbility','USkillDefinition>USkillAction','USkillAbility>USkillAction','USkillAction>USkillAbility','USkillSequenceAction>USkillAction','USkillParallelAction>USkillAction','USkillRepeatAction>USkillAction','UPdAbilitySystemComponent>UStatUpgradeDefinition','UPdAbilitySystemComponent>UBasicAttributeSet','UPandoraSkillSource>UPdAbilitySystemComponent','UReactiveStatusEffectAbility>UBasicAttributeSet','UStatusEffectWidget>UStatusEffectReplicationComponent'])assert(edges.has(id),`Missing current GAS path: ${id}`);
const sourceText=p=>source(p).join('\n');
const skillAbility=sourceText(nodes.get('USkillAbility').path.replace(/\.h$/,'.cpp'));
assert(skillAbility.includes('DuplicateObject<USkillAction>(Definition->Action, this)'));
assert(skillAbility.includes('DurationEndTime = Definition && Definition->SkillType == ESkillType::Duration'));
assert(skillAbility.includes('!bSkillCommitted || UsesSinceCooldown != 0 || !Spec || Spec->PendingRemove'));
const cooldown=sourceText(nodes.get('UPandoraSkillSource').path.replace(/\.h$/,'.cpp'));
assert(cooldown.includes('Query.EffectSource = this')&&cooldown.includes('GetActiveEffectsTimeRemainingAndDuration(Query)'));
const asc=sourceText(nodes.get('UPdAbilitySystemComponent').path.replace(/\.h$/,'.cpp'));
assert(asc.includes('Definition.CalculateInitialAttributeValues(InitialValues, ResourcesToFill)'));
const init=asc.slice(asc.indexOf('bool UPdAbilitySystemComponent::ApplyConfiguredAttributeDefaults'),asc.indexOf('bool UPdAbilitySystemComponent::ApplyAttributeDefaultValue'));
assert.equal((init.match(/ForceNetUpdate\(/g)||[]).length,1);
assert(init.indexOf('ResolvedDefaults.Emplace')<init.indexOf('SetNumericAttributeBase'));
